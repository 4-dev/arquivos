...
public class HttpClientUtil {
	...
	public static String doPost(String url, String jsonParam) {
		...
		try {
			...
			RequestConfig config = RequestConfig.custom().setSocketTimeout(3 * 1000).setConnectTimeout(3 * 1000)
					.build();
			...
			if (response.getStatusLine().getStatusCode() != 200) {
				throw new RuntimeException("doPost Failed : [URL:" + url + "] HTTP error code : "
			...
			System.out.println(
					"doPost Failed : [URL:" + url + "] HTTP error code : " + response.getStatusLine().getStatusCode());
